...
if __name__ == "__main__":
    ...
    Sp= 0.875; EOL= 0.80
    ### OptParam: OptXStamp:   4// OptYStamp:   1// OptHiddenSize:  94// OptBatchSzie: 37// OptDropoutRate: 0.036550// OptEpoch: 930
    OptXTimeStamps= 4; OptYtimeStamps= 1; OptBatchSize= 97; Epoch= 10000; LearningRate= 0.0000001; EarlyStopEpoch= 15
    plt.figure(figsize= (10, 4))
    ...
    if ModelOptimizationProcess== True:
        ...
        ParamBounds= [(4, 8), (1, 3), (6, 128), (8, 128), (0.001, 0.1), (10, 1000)]
        ...
        Battery3= ChargingPolicy1Data[["Cycle", "#3_SOH"]]
        Battery3= Battery3[Battery3["#3_SOH"]>= EOL]
        Battery3= Battery3.dropna(axis= 0)
        Battery3DischargeCapacity= np.array(Battery3["#3_SOH"])
        Battery3Cycle= np.array(Battery3["Cycle"])
        ...
        for Tindex in range(0, len(Battery3DischargeCapacity)):
            TargetSOH= Battery3DischargeCapacity[Tindex]
            ...
            if TargetSOH <= Sp and RulActivation== True:
                ...
                plt.figure(figsize= (10, 4))
                plt.title("Charging Policy #3", fontsize= 20)
                plt.plot(Battery3Cycle[: len(SOHls)], SOHls, label= "Battery#3 (online prediction)")
                plt.scatter(Battery3Cycle[len(SOHls)], Battery3DischargeCapacity[len(SOHls)], 
                label= "Tsp: #%s cycle"%(Tsp), color= "black", s= 50)
                plt.scatter(Battery3Cycle[KneePoint], Battery3DischargeCapacity[KneePoint], 
                ...
                RulActivation= False
        plt.figure(figsize= (10, 4))
        plt.plot(Battery3Cycle[: Tsp], SOHls[: Tsp], label= "Actual Battery#3 (known)", color= "black")
        plt.plot(Battery3Cycle[Tsp: ], SOHls[Tsp: ], label= "Actual Battery#3 (unknown)", color= "darkblue")
        plt.scatter(Battery3Cycle[Tsp], Battery3DischargeCapacity[Tsp], label= "Tsp: #%s cycle"%(Tsp), color= "black", s= 80)
        plt.plot(np.arange(Tsp, Tsp+ len(TargetPredictedSOH)), TargetPredictedSOH, label= "Multiple step-ahead prediciton results")
        plt.hlines(EOL, Battery3Cycle[0], Battery3Cycle[-1], label= "EOL (SOH= %s)"%(str(Sp)), color= "red", ls= "--")
        plt.grid(True)
        plt.xticks(fontsize= 20)
        plt.yticks(fontsize= 20)
        plt.xlabel("Cycle", fontsize= 20)
        ...
        plt.show()
        ### RUL computation
        ActualRul= len(Battery3DischargeCapacity)- Tsp
        PredictedRul= len(TargetPredictedSOH)
        ActualEol= len(Battery3DischargeCapacity)
