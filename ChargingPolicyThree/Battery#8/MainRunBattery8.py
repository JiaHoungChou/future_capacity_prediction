...
class ModelOptimization:
    ...
    def DeepLearningTrochTraining(self, XStamps, YStamps, HiddenSize, BatchSzie, DropoutRate, Epoch):
        ...
        if self.InitOptimizationLoss== None:
            ...
            if self.RollingPredictionVisualization!= False:
                ...
                plt.plot(np.arange(0, len(MutipleStepsRollingPredictionResults)), MutipleStepsRollingPredictionResults, label= "Online predicted history SOH", color= "darkred")
                plt.plot(np.arange(0, len(self.SingleVariateTrainX)), self.SingleVariateTrainX, label= "Actual history SOH", color= "darkblue")
                plt.grid(True)
    ...
    def OptimizationProcessMainRun(self, ParamBounds, OptMaxiter, Visualization= True):
        ...
        if Visualization!= False:
            ...
            plt.plot(np.arange(0, len(MutipleStepsRollingPredictionResults)), MutipleStepsRollingPredictionResults, label= "Mutiple Step Prediction")
...
if __name__ == "__main__":
    ...
    Sp= 0.860; EOL= 0.800
    # OptParam: OptXStamp:   5// OptYStamp:   1// OptHiddenSize:  71// OptBatchSzie:  36// OptDropoutRate: 0.000000// OptEpoch: 850
    ...
    plt.plot(ChargingPolicy3Data["Cycle"], ChargingPolicy3Data["Battery#7"], label= "Battery#1")
    plt.plot(ChargingPolicy3Data["Cycle"], ChargingPolicy3Data["Battery#8"], label= "Battery#2")
    plt.plot(ChargingPolicy3Data["Cycle"], ChargingPolicy3Data["Battery#9"], label= "Battery#3")
    plt.grid(True)
    ...
    if ModelOptimizationProcess== True:
        ...
        ParamBounds= [(4, 12), (1, 3), (12, 100), (8, 152), (0.01, 0.1), (50, 1000)]
        MutipleStepsRollingPredictionResults= ModelOptimizaionModule.OptimizationProcessMainRun(ParamBounds= (ParamBounds), OptMaxiter= 30, Visualization= True)
        ...
        files= os.listdir(os.path.join(os.getcwd(), "ModelFinTuneCheckPoint"))
        for f in files:
            os.remove(os.path.join(os.getcwd(), "ModelFinTuneCheckPoint", f))
        ### Transfer learning process
        Battery8= ChargingPolicy3Data[["Cycle", "#8_SOH"]]
        Battery8= Battery8.dropna(axis= 0)
        Battery8DischargeCapacity= np.array(Battery8["#8_SOH"])
        Battery8Cycle= np.array(Battery8["Cycle"])
        ...
        for Tindex in range(0, len(Battery8DischargeCapacity)):
            TargetSOH= Battery8DischargeCapacity[Tindex]
            ...
            if TargetSOH <= Sp and RulActivation== True:
                ...
                KneeOnsetTransfer= np.where(np.array(SOHls)<= SourceDtaKneeOnsetValue)[0][0]
                plt.figure(figsize= (10, 4))
                # plt.title("Charging Policy #3", fontsize= 20)
                plt.plot(Battery8Cycle[: len(SOHls)], SOHls, label= "Battery#9 (online prediction)")
                plt.scatter(Battery8Cycle[len(SOHls)], Battery8DischargeCapacity[len(SOHls)], 
                label= "Tsp: #%s cycle"%(Tsp), color= "black", s= 50)
                plt.scatter(Battery8Cycle[KneePoint], Battery8DischargeCapacity[KneePoint], 
                label= "Knee-Point: #%s cycle"%(KneePoint), color= "darkblue", s= 50)
                plt.scatter(Battery8Cycle[KneeOnsetTransfer], Battery8DischargeCapacity[KneeOnsetTransfer], 
                label= "Transfered Knee-Onset: #%s cycle"%(KneeOnsetTransfer), color= "darkred", s= 50)
                plt.grid(True)
                plt.xlabel("Cycle", fontsize= 20)
                plt.ylabel("Discharge Capacity", fontsize= 20)
                plt.xticks(fontsize= 20)
                ...
                RulActivation= False
        plt.figure(figsize= (8, 4))
        plt.plot(Battery8Cycle[: Tsp], SOHls[: Tsp], label= "Actual Battery#8 (known)", color= "black")
        plt.plot(Battery8Cycle[Tsp: ], SOHls[Tsp: ], label= "Actual Battery#8 (unknown)", color= "darkblue")
        plt.scatter(Battery8Cycle[Tsp], Battery8DischargeCapacity[Tsp], label= "Tsp: #%s cycle (SOH= %s)"%(Tsp, str(Sp)) , color= "black", s= 80)
        ...
        plt.hlines(0.80, Battery8Cycle[0], Battery8Cycle[-1], label= "EOL (SOH= %s)"%(str(EOL)), color= "red", ls= "--")
        plt.grid(True)
        plt.xticks(fontsize= 18)
        plt.yticks(fontsize= 18)
        ...
        plt.show()
        ### RUL computation
        ActualRul= len(Battery8DischargeCapacity)- Tsp
        PredictedRul= len(TargetPredictedSOH)
        ActualEol= len(Battery8DischargeCapacity)
