...
class ModelOptimization:
    ...
    def DeepLearningTrochTraining(self, XStamps, YStamps, HiddenSize, BatchSzie, DropoutRate, Epoch):
        ...
        if self.InitOptimizationLoss== None:
            ...
            if self.RollingPredictionVisualization!= False:
                ...
                plt.plot(np.arange(0, len(MutipleStepsRollingPredictionResults)), MutipleStepsRollingPredictionResults, label= "mutistep-ahead rolling prediction evaluation", color= "darkred")
        ...
        self.OptimizationLoss.append(ThreePerformanceMetric)
    ...
    def OptimizationProcessMainRun(self, ParamBounds, OptMaxiter, Visualization= True):
        ...
        if Visualization!= False:
            plt.figure(figsize= (10, 4))
            # plt.title("Mutiple Steps Model Rolling Prediction (Optimized Model)", fontsize= 20, fontweight= "bold")
            plt.plot(np.arange(0, len(MutipleStepsRollingPredictionResults)), MutipleStepsRollingPredictionResults, label= "mutistep-ahead rolling predicted SOH", color= "darkblue")
            plt.plot(np.arange(0, len(self.SingleVariateTrainX)), self.SingleVariateTrainX, label= "actual SOH", color= "darkred")
            plt.xlabel("Cycle", fontsize= 20)
            plt.ylabel("SOH", fontsize= 20)
            plt.legend(loc= "best", fontsize= 20)
            plt.grid(True)
...
if __name__ == "__main__":
    torch.cuda.empty_cache()
    MainPath= os.getcwd()
    FileName= "ChargingPolicy3.csv"
    with open(os.path.join(MainPath, FileName), "r") as file:
        ChargingPolicy3Data= pd.read_csv(open(os.path.join(MainPath, FileName)))
    ModelOptimizationProcess= False
    Sp= 0.860; EOL= 0.800
    # OptParam: OptXStamp:   7// OptYStamp:   1// OptHiddenSize:  26// OptBatchSzie:  77// OptDropoutRate: 0.000000// OptEpoch: 565
    OptXTimeStamps= 5; OptYtimeStamps= 1; OptBatchSize= 70; Epoch= 10000; LearningRate= 0.01; EarlyStopEpoch= 10
    plt.figure(num= 1, figsize= (10, 4))
    # plt.title("Charging Policy #3", fontsize= 20)
    plt.plot(ChargingPolicy3Data["Cycle"], ChargingPolicy3Data["Battery#7"], label= "Battery#7")
    plt.plot(ChargingPolicy3Data["Cycle"], ChargingPolicy3Data["Battery#8"], label= "Battery#8")
    plt.plot(ChargingPolicy3Data["Cycle"], ChargingPolicy3Data["Battery#9"], label= "Battery#9")
    plt.grid(True)
    ...
    Battery7= ChargingPolicy3Data[["Cycle", "#7_SOH"]]
    Battery7= Battery7.dropna(axis= 0)
    Battery7DischargeCapacity= np.array(Battery7["#7_SOH"])
    Battery7Cycle= np.array(Battery7["Cycle"])
    ### Knee-onset computation
    ParameterBound= [1, -1e-4, -1e-4, len(Battery7DischargeCapacity)* 0.7]
    Coe, Cov= curve_fit(BaconWattsKneeOpintModel, Battery7Cycle, Battery7DischargeCapacity, ParameterBound)
    ...
    Coe, Cov= curve_fit(DoubleBaconWattsModel, Battery7Cycle, Battery7DischargeCapacity, ParameterBound)
    ...
    plt.figure(figsize= (10, 4))
    # plt.title("Charging Policy #3", fontsize= 20)
    plt.plot(Battery7Cycle, Battery7DischargeCapacity, label= "Battery#7")
    plt.scatter(Battery7Cycle[KneeOnsetByDoubleBaconWatts], Battery7DischargeCapacity[KneeOnsetByDoubleBaconWatts], 
    label= "Knee-onset: #%s cycle"%(KneeOnsetByDoubleBaconWatts), color= "black", s= 50)
    plt.scatter(Battery7Cycle[-1], Battery7DischargeCapacity[-1], 
    label= "EOL: #%s cycle"%(Battery7Cycle[-1]), color= "darkblue", s= 50)
    plt.grid(True)
    plt.xticks(fontsize= 20)
    plt.yticks(fontsize= 20)
    plt.xlabel("Cycle", fontsize= 20)
    ...
    if ModelOptimizationProcess== True:
        ...
        SingleVariateTraningSet= Battery7DischargeCapacity[KneeOnsetByDoubleBaconWatts+ 1: ]
        ...
        ParamBounds= [(6, 12), (1, 5), (12, 100), (8, 128), (0.01, 0.1), (100, 1000)]
        ...
        plt.figure(figsize= (10, 4))
        plt.plot(np.arange(0, len(ModelOptimizaionModule.OptimizationLoss)), ModelOptimizaionModule.OptimizationLoss, label= "hybridized loss", color= "darkblue")
        plt.scatter(np.arange(0, len(ModelOptimizaionModule.OptimizationLoss)), ModelOptimizaionModule.OptimizationLoss, color= "darkblue", edgecolors= "black", s= 50)
        plt.grid(True)
        plt.xticks(fontsize= 18)
        plt.yticks(fontsize= 18)
        ...
        Battery9= ChargingPolicy3Data[["Cycle", "#9_SOH"]]
        Battery9= Battery9.dropna(axis= 0)
        Battery9DischargeCapacity= np.array(Battery9["#9_SOH"])
        Battery9Cycle= np.array(Battery9["Cycle"])
        ### Online simulation process
        SourceDtaKneeOnsetValue= Battery7DischargeCapacity[KneeOnsetByDoubleBaconWatts]
        StopRule= 0.80; NowIndex= 0; SOHls= []; RulActivation= True; Tsp= None
        for Tindex in range(0, len(Battery9DischargeCapacity)):
            TargetSOH= Battery9DischargeCapacity[Tindex]
            ...
            if TargetSOH <= Sp and RulActivation== True:
                ...
                plt.figure(figsize= (10, 4))
                # plt.title("Charging Policy #3", fontsize= 20)
                plt.plot(Battery9Cycle[: len(SOHls)], SOHls, label= "Battery#9 (online prediction)")
                plt.scatter(Battery9Cycle[len(SOHls)], Battery9DischargeCapacity[len(SOHls)], 
                label= "Tsp: #%s cycle"%(Tsp), color= "black", s= 50)
                plt.scatter(Battery9Cycle[KneePoint], Battery9DischargeCapacity[KneePoint], 
                label= "Knee-PointOnset: #%s cycle"%(KneePoint), color= "darkblue", s= 50)
                plt.grid(True)
                plt.xlabel("Cycle", fontsize= 20)
                ...
                RulActivation= False
        plt.figure(figsize= (8, 4))
        plt.plot(Battery9Cycle[: Tsp], SOHls[: Tsp], label= "Actual Battery#9 (known)", color= "black")
        plt.plot(Battery9Cycle[Tsp: ], SOHls[Tsp: ], label= "Actual Battery#9 (unknown)", color= "darkblue")
        plt.scatter(Battery9Cycle[Tsp], Battery9DischargeCapacity[Tsp], label= "Tsp: #%s cycle (SOH= %s)"%(Tsp, str(Sp)) , color= "black", s= 80)
        plt.plot(np.arange(Tsp, Tsp+ len(TargetPredictedSOH)), TargetPredictedSOH, label= "Multiple step-ahead prediciton results", color= "red")
        plt.hlines(0.80, Battery9Cycle[0], Battery9Cycle[-1], label= "EOL (SOH= %s)"%(str(EOL)), color= "red", ls= "--")
        plt.grid(True)
        plt.xticks(fontsize= 18)
        plt.yticks(fontsize= 18)
        ...
        plt.show()
        ### RUL computation
        ActualRul= len(Battery9DischargeCapacity)- Tsp
        PredictedRul= len(TargetPredictedSOH)
        ActualEol= len(Battery9DischargeCapacity)
