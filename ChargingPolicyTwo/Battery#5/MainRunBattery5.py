...
if __name__ == "__main__":
    ...
    OptXTimeStamps= 14; OptYtimeStamps= 1; OptBatchSize= 113; Epoch= 1000; LearningRate= 0.00001; EarlyStopEpoch= 10
    ...
    Sp= 0.860; EOL= 0.75
    plt.figure(figsize= (10, 4))
    plt.plot(ChargingPolicy2Data["Cycle"][ChargingPolicy2Data["#4_SOH"]>= EOL], ChargingPolicy2Data["#4_SOH"][ChargingPolicy2Data["#4_SOH"]>= EOL], label= "Battery#4")
    plt.plot(ChargingPolicy2Data["Cycle"][ChargingPolicy2Data["#5_SOH"]>= EOL], ChargingPolicy2Data["#5_SOH"][ChargingPolicy2Data["#5_SOH"]>= EOL], label= "Battery#5")
    plt.plot(ChargingPolicy2Data["Cycle"][ChargingPolicy2Data["#6_SOH"]>= EOL], ChargingPolicy2Data["#6_SOH"][ChargingPolicy2Data["#6_SOH"]>= EOL], label= "Battery#6")
    plt.grid(True)
    plt.xlabel("Cycle", fontsize= 20)
    plt.xticks(fontsize= 20)
    plt.yticks(fontsize= 20)
    ...
    if ModelOptimizationProcess== True:
        ...
        ParamBounds= [(10, 18), (1, 9), (12, 100), (8, 256), (0.01, 0.1), (100, 1000)]
        MutipleStepsRollingPredictionResults= ModelOptimizaionModule.OptimizationProcessMainRun(ParamBounds= (ParamBounds), OptMaxiter= 50, Visualization= True)
        ...
        Battery5= ChargingPolicy2Data[["Cycle", "#5_SOH"]]
        Battery5= Battery5[Battery5["#5_SOH"]>= EOL]
        Battery5= Battery5.dropna(axis= 0)
        Battery5DischargeCapacity= np.array(Battery5["#5_SOH"])
        Battery5Cycle= np.array(Battery5["Cycle"])
        ...
        for Tindex in range(0, len(Battery5DischargeCapacity)):
            TargetSOH= Battery5DischargeCapacity[Tindex]
            ...
            if TargetSOH <= Sp and RulActivation== True:
                ...
                plt.figure(figsize= (10, 4))
                plt.title("Charging Policy #3", fontsize= 20)
                plt.plot(Battery5Cycle[: len(SOHls)], SOHls, label= "Battery#5 (online prediction)")
                plt.scatter(Battery5Cycle[len(SOHls)], Battery5DischargeCapacity[len(SOHls)], 
                label= "Tsp: #%s cycle"%(Tsp), color= "black", s= 50)
                plt.scatter(Battery5Cycle[KneePoint], Battery5DischargeCapacity[KneePoint], 
                ...
                RulActivation= False
        plt.figure(figsize= (10, 4))
        plt.plot(Battery5Cycle[: Tsp], SOHls[: Tsp], label= "Actual Battery#5 (known)", color= "black")
        plt.plot(Battery5Cycle[Tsp: ], SOHls[Tsp: ], label= "Actual Battery#5 (unknown)", color= "darkblue")
        plt.scatter(Battery5Cycle[Tsp], Battery5DischargeCapacity[Tsp], label= "Tsp: #%s cycle"%(Tsp), color= "black", s= 80)
        plt.plot(np.arange(Tsp, Tsp+ len(TargetPredictedSOH)), TargetPredictedSOH, label= "Multiple step-ahead prediciton results")
        plt.hlines(EOL, Battery5Cycle[0], Battery5Cycle[-1], label= "EOL (SOH= %s)"%(str(Sp)), color= "red", ls= "--")
        plt.grid(True)
        plt.xticks(fontsize= 20)
        plt.yticks(fontsize= 20)
        plt.xlabel("Cycle", fontsize= 20)
        ...
        plt.show()
        ### RUL computation
        ActualRul= len(Battery5DischargeCapacity)- Tsp
        PredictedRul= len(TargetPredictedSOH)
        ActualEol= len(Battery5DischargeCapacity)
